...
#### Langkah Pembuatan:
1. Masuk ke repo kelompok Anda, Klik menu dropdown branch di kiri atas (yang bertuliskan **`main`**).
2. Ketik nama branch baru Anda dengan format: `[NIM-Nama]` *(Contoh: `244311058-RiyanZakaria`)* , Klik **Create branch: 240001-Ahmad from 'main'**.
...
5. Pastikan konfigurasi perbandingan branch:  
   * **`base: main`** -, **`compare: [NIM-Nama]`** *(Contoh: `compare: 244311058-RiyanZakaria`)*
...
2. Masuk ke branch yang berisi berkas:
   ...
   * `02_INTERAKSI_PANITIA.md` (Daftar Seluruh Panitia & Kolom TTD)
   * `03_INTERAKSI_ANGKATAN.md` (Daftar Mahasiswa Baru 1 Angkatan)
   * Sub-folder `assets/`
3. Buka file yang ingin dikerjakan, Klik ikon ✏️ **Pensil (Edit this file)**.
...
* **Cara Upload:**
  ...
  4. Beri pesan commit: `upload foto panitia`, Klik **Commit changes**.
...
1. Syarat Sebelum Menemui Panitia: Sebelum menemui Panitia, berkas 02_INTERAKSI_PANITIA.md pada branch pribadi Anda MINIMAL sudah terisi Nama Lengkap dan Divisi Panitia yang bersangkutan.
2. Saat Menemui Panitia: Lakukan interaksi dan foto bersama Panitia. Tunjukkan Link Pull Request (PR) atau Repo  milik Anda kepada Panitia melalui perangkat HP.
3. Proses Pembubuhan TTD oleh Panitia: Panitia membuka link PR melalui HP -> Masuk ke tab 'Files changed' -> Mencari file 02_INTERAKSI_PANITIA.md -> Klik ikon plus (+) pada baris target komentar TTD (<!-- TARGET_LINE_COMMENT_TTD_PANITIA_XX -->) -> Mengetikkan format baku:
[SIGNED] [Nama_Panitia] - Divisi [Nama_Divisi]
-> Menekan tombol 'Add single comment'.
